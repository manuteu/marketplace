...
export default function ProductCard({ name, is_new, price, is_active, product_images, user, ...rest }: Props) {
  ...
  return (
    <Pressable {...rest} style={{
      ...
      <VStack mt={1}>
        <Text fontFamily='regular' fontSize='sm' color={is_active ? 'gray.600' : 'gray.400'} >{name}</Text>
        <HStack style={{ gap: 2 }} mt={-1}>
          <Text fontFamily='bold' fontSize='xs' color={is_active ? 'gray.700' : 'gray.400'} mt={1}>R$</Text>
          <Text fontFamily='bold' fontSize='md' color={is_active ? 'gray.700' : 'gray.400'}>{formatMoney(price)}</Text>
        </HStack>
      </VStack>
    </Pressable>
  )
